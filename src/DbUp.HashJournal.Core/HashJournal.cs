...
namespace DbUp.HashJournal.Core
{
    public abstract class HashJournal : IJournal
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="HashJournal"/> class.
        /// </summary>
        ...
        /// <summary>
        /// Separator between name and hash value returned in GetExecutedScripts
        /// </summary>
            ...
            return ConnectionManager().ExecuteCommandsWithManagedConnection(dbCommandFactory =>
            {
                if (journalExists || DoesTableExist(dbCommandFactory))
                {
                    ...
                    using (var command = GetJournalEntriesCommand(dbCommandFactory))
                    {
                        ...
                        {
                            while (reader.Read())
                                scripts.Add((string)reader[0] + NameHashSeparator + (string)reader[1]);
                        }
                    }
        ...
        protected IDbCommand GetInsertScriptCommand(Func<IDbCommand> dbCommandFactory, SqlScript script)
        {
            ...
            hashParam.Value = MD5Hasher.GetHash(script.Contents);
            
            command.CommandText = GetInsertJournalEntrySql("@scriptName", "@applied", "@hash");
        ...
        /// <param name="hash">Name of the hash param (i.e @hash)</param>
        /// <returns></returns>
        protected abstract string GetInsertJournalEntrySql(string @scriptName, string @applied, string @hash);
